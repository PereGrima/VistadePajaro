...
```{=html}
<div id="tbl-tabla-hijos" class="tabla-wrapper35">
...
![Distribución de los salarios en España en 2021. Fuente: [INE](https://www.ine.es/prensa/ees_2021.pdf)](Figuras/f0201_distribucionSalariosINE.png){#fig-salariosINE .fig-normal1 fig-align="center" width="85%"}
...
![Situación de la mediana y de los cuartiles en un conjunto de 30 datos.](Figuras/f0202_Cuartiles2.png){#fig-cuartiles .fig-normal2 fig-align="center" width="67%"}
...
La media ponderada se realiza cuando no se da el mismo peso a todos los valores. Si en una asignatura se tienen dos notas, 6 y 3, la media aritmética es 4,5, pero si se calcula una media ponderada de manera que la primera tenga el doble de peso que la segunda, el resultado es:
$$ \bar{x}_p = \frac{2\cdot 6+1 \cdot 3}{3}=5$$

En general, siendo $x_i$ los valores y $p_i$ los pesos asignados a cada valor, tenemos:
...
La media geométrica ($MG$) de un conjunto de valores $x_1, x_2, \cdots , x_n$ es:
...
Si en un viaje de ida y vuelta realiza la ida a 100 km/h y la vuelta, por el mismo camino, a 120 km/h su velocidad media no es 110 km/h. En efecto, sea $d$ la distancia tanto de ida como de vuelta. El tiempo tardado en la ida es $t_1=d/100$ y en la vuelta $t_2=d/120$ por tanto, la velocidad global será:
$$ v = \frac{2d}{\frac{d}{100}+\frac{d}{120}} = \frac{2}{\frac{1}{100}+\frac{1}{120}} = 109.1 \text{ km/h} $$
...
Si se introducen estos valores en una calculadora se obtiene: 
$$ \text{Media} = 8.52857...\text{ kg} \quad \quad  \text{Desv. típica} = 0,33022...\text{ kg}$$
...
![Media y desviación típica de los valores sin redondear y redondeados calculados con Excel. Los pesos reales se han generado con la expresión: =8+aleatorio().](Figuras/f0203_valoresRedondeados.png){#fig-valoresRedondeados .fig-normal2 fig-align="center" width="45%"}
...
Los problemas con porcentajes pueden parecer triviales y no serlo. Uno de los más famosos es la llamada “paradoja de las patatas”: supongamos que tenemos 100 kg de patatas cuyo peso está compuesto por agua en un 99%. Con el paso del tiempo las patatas pierden humedad ¿Cuánto pesarán cuando el porcentaje de humedad sea del 98%?[^20_Patatas].

[^20_Patatas]: <small>Después de secarse sigue habiendo 1 kg de "polvo de patata"; si ese kg es un 2% del total, el total son 50 kg.</small>\
...
```{=html}
<div id="tbl-Simpson"; class="tabla-wrapper42">
...
El promedio coincide con $\mu$. Esto pasa en todos los casos, independientemente del tipo de población o del tamaño de la muestra.

Veamos ahora si el estadístico:
$$  S_{n}^{2}= \frac{ \sum_{i=1}^{n}{(x_i-\bar{x})^2} }{n} $$
donde $n$ es el tamaño de la muestra, es un estimador insesgado para la varianza $\sigma_{N}^{2}$, calculada como:
