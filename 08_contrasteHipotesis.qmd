...
Rudolf Wolf (1816-1893) fue un astrónomo suizo conocido por sus investigaciones sobre las manchas solares. También tenía interés --los científicos de la época solían tener una amplia gama de intereses-- en el cálculo de probabilidades. Algunas fuentes[^08_contrastehipotesis-1] indican que, para verificar el cumplimiento de las leyes del cálculo de probabilidades, lanzó 20.000 veces un par de dados --uno blanco y otro rojo-- obteniendo los resultados que se muestran en la [@fig-tablaDadosWolf].

[^08_contrastehipotesis-1]: Nosotros descubrimos estos datos en el libro de M. G. Bulner: Principles of Statistics, Ed. Dover, 1979. La publicación original que se cita (Czuber, 1903) puede consultarse [aquí](https://archive.org/details/wahrscheinlichk01czubgoog), pág. 118).

![Resultados obtenidos por R. Wolf al lanzar 20.000 veces dos dados (Reproducido de: M. G. Bulner: Principles of Statistics, Ed. Dover, 1979).](Figuras_08/f0801_tablaDadosWolf.png){#fig-tablaDadosWolf .fig-normal_0801 fig-align="center"}
...
![Resultados obtenidos en el experimento de Wolf. Dado B: blanco, R: rojo.).](Figuras_08/f0802_grafDadosWolf.png){#fig-grafDadosWolf .fig-normal_0802 fig-align="center"}
...
![Simulación con una hoja de cálculo de la discrepancia máxima entre la frecuencia observada y la esperada de los resultados de lanzar $20\;\!000$ veces un dado.](Figuras_08/f0803_excelDados.png){#fig-excelDados .fig-normal_0803 fig-align="center"}

Para repetir la simulación y obtener otro valor de la discrepancia máxima, basta con pulsar [F9]{style="font-family: monospace;"} si se usa Excel o [Ctr+R]{style="font-family: monospace;"} en Google Sheets. Podemos ir anotando los valores que van saliendo[^08_contrastehipotesis-2]. La [Tabla 8.2](#tbl-excelDados) contiene los resultados obtenidos en tres simulaciones destacando en negrita el valor de la discrepancia máxima en cada caso.

[^08_contrastehipotesis-2]: Tal como se ha hecho en el Apéndice 7.B para simular el proceso de estimación del número de taxis que hay en una ciudad.
...
![Aumento de variabiilidad por sobreajuste. Cambiar la media de la distribución para corregir falsos desajustes aumenta la variabilidad global.](Figuras_08/f0805_sobreAjuste.png){#fig-sobreAjuste .fig-normal_0805 fig-align="center"}
...
![Nuestros valores comparados con el histograma de pesos obtenidos cuando todo va bien.](Figuras_08/f0806_pesosCafe_1.png){#fig-pesosCafe_1 .fig-normal_0806 fig-align="center"}
...
![Probabilidad de obtener valores menores que los indicados](Figuras_08/f0807_pesosCafe_2.png){#fig-pesosCafe_2 .fig-normal_0807 fig-align="center"}
...
![El estadístico de prueba puede caer en la zona de rechazo siendo cierta $\text{H}_0$ (error tipo I, parte superior) o puede no llegar a esa zona siendo falsa (error tipo II, parte inferior).](Figuras_08/f0808_errorTipoII.png){#fig-errorTipoII .fig-normal_0808 fig-align="center"}
...
![Distribución de las observaciones individuales (azul) y de las medias de 4 observaciones (rojo) con un valor perteneciente a cada distribución.](Figuras_08/f0809_distMedia.png){#fig-distMedia .fig-normal_0809 fig-align="center"}
...
![Probabilidades de error tipo I y tipo II controlando observaciones individuales o medias de 4 unidades.](Figuras_08/f0810_errorTipoIImedias.png){#fig-errorTipoIImedias .fig-normal_0810 fig-align="center"}
...
<tbody>
<tr>
  <td colspan="3">20 lanzamientos</td>
</tr>
...
Lo primero --y lo más laborioso y delicado-- será planificar cómo se van a obtener los resultados de la cosecha y vigilar que todo el proceso se realice de la forma prevista hasta pesar los tomates. En principio, habrá que utilizar cuantas más plantas mejor[^08_contrastehipotesis-3], pero siempre hay limitaciones (de espacio, de tiempo que podemos dedicar,...) y es mejor tener pocos datos recogidos con cuidado que tener muchos que no son fiables. Pongamos que podemos usar 20 plantas y las repartimos en dos grupos de 10. A las de un grupo no le pondremos abono, será el grupo de control, y al otro sí, será el grupo tratado.

[^08_contrastehipotesis-3]: Veremos que se puede calcular el número de plantas necesario suponiendo una cierta variabilidad en los resultados y de acuerdo con unas probabilidades de error tipo I y tipo II previamente fijadas.
...
En una situación como la que estamos analizando basta con realizar diagramas de puntos como los de la [@fig-dotplot8]. La línea roja representa el valor medio de cada grupo.

![Representación gráfica del peso de los tomates obtenidos en cada planta según pertenezcan al grupo sin abono (Control) o con abono (Tratado).](Figuras_08/f0811_dotplot.png){#fig-dotplot8 .fig-normal_0811 fig-align="center"}

El gráfico da la sensación de que las plantas con abono (representadas por el grupo tratado) tienen --en promedio-- más producción que las no abonadas (representadas por el grupo de control[^08_contrastehipotesis-4]). La situación estaría mucho más clara si nos encontráramos en una de las situaciones que se muestran en la [@fig-dotplot2]. Si los datos del grupo tratado son los que corresponden a la situación A no podremos afirmar que el abono aumenta la cosecha, aunque la media de la producción es ligeramente superior que en el grupo tratado (5,2 frente a 5,1 kg). Por el contrario, si los resultados fueran los representados en la situación B, podríamos afirmar sin temor a equivocarnos que la producción es mayor con abono.

[^08_contrastehipotesis-4]: Los datos que tenemos son solo los representantes de los grupos de interés, que son las producciones "en general" de las plantas con y sin abono.

![No sería necesario seguir analizando los datos si los resultados fueran los de la situación A o B.](Figuras_08/f0812_dotplot2.png){#fig-dotplot2 .fig-normal_0812 fig-align="center"}
...
```{=html}
<div id="tbl-textDSNO" class="tabla-wrapper_T0806">
...
![Cálculo del $p$-valor colocando el estadístico de prueba en la distribución de referencia.](Figuras_08/f0814_distRefAlea.png){#fig-distRefAlea .fig-normal_0814 fig-align="center"}
...
La [@fig-distRefPermu] se ha construido con valores obtenidos generando un millón de ordenaciones al azar. Han aparecido $194\;\!123$ valores[^08_contrastehipotesis-6] mayores o iguales al estadístico de prueba (1,046). El $p$-valor resultante es en este caso del 1,9 %.

[^08_contrastehipotesis-6]: En otra simulación este número no sería idéntico pero sí muy similar.

![Otro test de comparación de medias. Distribución de referencia y determinación del $p$-valor.](Figuras_08/f0815_distRefPermu.png){#fig-distRefPermu .fig-normal_0815 fig-align="center"}
...
![Distribución de los nacidos en la primera mitad del año en una muestra de $n=516$ y $p = 0{,}5$. Probabilidad de que tener un valor igual o superior a 308.](Figuras_08/f0816_distRefFutbol.png){#fig-distRefFutbol .fig-normal_0816 fig-align="center"}

El cálculo de esta probabilidad se puede realizar fácilmente dado que se trata de una variable aleatoria con distribución binomial: $$\text{P}(X \geq 308) = 1 - \text{B}(x=307; n=516; p = 0.5) = 6.18 \cdot 10^{-6}$$
...
Si solo consideramos los jugadores cuya primera nacionalidad es la española tenemos un total de 292 de los cuales 170 (el 58,2%) nacieron en la primera mitad del año. Con estos nuevos datos, la probabilidad de que se dé ese desequilibrio entre los nacidos entre el primer y el segundo semestre del año es: $$P(X \geq 170) = 1 - B(x=169; \;n=292; \; p = 0{,}514) = 0{,}011$$ Si la teoría es correcta y en las categorías infantiles los mayores son los que tienden a triunfar, la presencia de jugadores formados en sistemas donde los mayores corresponden a nacidos en la segunda mitad del año distorsiona nuestros datos y dificulta observar con claridad el efecto buscado. Sin embargo, en la temporada analizada, al excluir a esos futbolistas del estudio, la proporción de nacidos en la primera mitad del año disminuye en lugar de aumentar, como cabría esperar.
...
![Distribución de referencia y estadístico de prueba en el estudio sobre aspirina y riesgo de infarto.](Figuras_08/f0817_destRef_aspirina.png){#fig-destRef_aspirina .fig-normal_0817 fig-align="center"}
...
![Árbol de probabilidades para el ejemplo del sorteo de lotería.](Figuras_08/f0818_Bayes.png){#fig-Bayes .fig-normal_0818 fig-align="center"}
